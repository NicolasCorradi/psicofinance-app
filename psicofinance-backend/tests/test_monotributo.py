...
def _mock_facturado(monkeypatch, valor: float) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda sb, hasta, user_id, criterio="DEVENGADO": valor,
    )
...
class TestSemaforoTransiciones:
    ...
    def test_verde_debajo_del_80(self, monkeypatch):
        ...
        r = obtener_semaforo(_fake_con_categoria("D"), "test-user")
...
class TestSumarFacturadoUSD:
    ...
    def test_turno_usd_convierte_con_tipo_cambio(self):
        ...
        total = sumar_facturado_ultimos_12_meses(sb, hasta=date(2026, 7, 1), user_id="test-user")
    ...
    def test_filtro_devengado_usa_fecha_turno(self):
        ...
        sumar_facturado_ultimos_12_meses(sb, hasta=date(2026, 7, 1), user_id="test-user")
    ...
    def test_filtro_percibido_usa_fecha_cobro(self):
        ...
        sb = FakeSupabase({"turnos": []})
        sumar_facturado_ultimos_12_meses(sb, hasta=date(2026, 7, 1), user_id="test-user", criterio="PERCIBIDO")
        tabla, params = sb.llamadas[0]
        assert tabla == "turnos"
    ...
    def test_sin_turnos_devuelve_cero(self):
        sb = FakeSupabase({"turnos": []})
        assert sumar_facturado_ultimos_12_meses(sb, hasta=date(2026, 7, 1), user_id="test-user") == 0
...
class TestCategoriaYTope:
    ...
    def test_categoria_invalida_cae_al_tope_de_config(self, monkeypatch):
        ...
        r = obtener_semaforo(_fake_con_categoria("Z"), "test-user")
    ...
    def test_tope_cero_no_divide_por_cero(self, monkeypatch):
        ...
        r = obtener_semaforo(_fake_con_categoria("Z"), "test-user")  # "Z" → tope de config → 0
