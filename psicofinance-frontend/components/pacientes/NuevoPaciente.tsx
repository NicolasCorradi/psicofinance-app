    ...
    <>
      ...
      <div className="fixed left-1/2 top-1/2 z-50 w-full max-w-sm -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-black/5">
        ...
        <div className="p-5">
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-2">
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-medium uppercase tracking-wider text-neutral-400">Nombre *</label>
                <input
                  autoFocus
                  value={form.nombre}
              ...
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-medium uppercase tracking-wider text-neutral-400">Apellido *</label>
                <input
                  value={form.apellido}
            ...
            <div className="flex flex-col gap-1">
              <label className="text-[10px] font-medium uppercase tracking-wider text-neutral-400">Email (opcional)</label>
              <input
                type="email"
                value={form.email ?? ""}
            ...
            <div className="flex flex-col gap-1">
              <label className="text-[10px] font-medium uppercase tracking-wider text-neutral-400">Honorario inicial $ (opcional)</label>
              <input
                type="number"
                min={1}
                value={form.honorario_actual ?? ""}
                onChange={(e) => setForm((f) => ({ ...f, honorario_actual: e.target.value ? Number(e.target.value) : null }))}
                className="rounded-lg border border-neutral-200 px-2.5 py-2 text-sm tabular-nums focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-100"
          ...
          {error && (
            <div className="mt-3 rounded-xl bg-red-50 px-3 py-2 ring-1 ring-red-100">
