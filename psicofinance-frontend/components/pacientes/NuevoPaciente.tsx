...
export default function NuevoPaciente({ onCreado, onCancelar }: Props) {
  ...
  return (
    <>
      ...
      {/* Modal */}
      <div className="fixed left-1/2 top-1/2 z-50 w-[calc(100%-2rem)] max-w-sm -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-2xl bg-white dark:bg-slate-900 shadow-2xl ring-1 ring-black/5 dark:ring-white/10">
        ...
        <div className="p-5">
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-2">
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-medium uppercase tracking-wider text-neutral-400 dark:text-slate-500">Nombre *</label>
                <input
                  autoFocus
                  value={form.nombre}
                  onChange={(e) => set("nombre", e.target.value)}
                  onKeyDown={(e) => e.key === "Enter" && guardar()}
                  className="rounded-lg border border-neutral-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-neutral-900 dark:text-slate-100 px-2.5 py-2 text-sm focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-100"
                  placeholder="María"
                />
              </div>
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-medium uppercase tracking-wider text-neutral-400 dark:text-slate-500">Apellido *</label>
                <input
                  value={form.apellido}
            ...
            <div className="flex flex-col gap-1">
              <label className="text-[10px] font-medium uppercase tracking-wider text-neutral-400 dark:text-slate-500">Email (opcional)</label>
              <input
                type="email"
                value={form.email ?? ""}
            ...
            <div className="flex flex-col gap-1">
              <label className="text-[10px] font-medium uppercase tracking-wider text-neutral-400 dark:text-slate-500">Celular (opcional)</label>
              <input
                type="tel"
                inputMode="tel"
                value={form.telefono ?? ""}
              ...
              />
              <p className="text-[10px] text-neutral-400 dark:text-slate-500">Para enviar recordatorios de pago por WhatsApp.</p>
            </div>
          ...
          <div className="mt-5 flex gap-2">
            ...
            <button
              onClick={onCancelar}
              className="rounded-xl border border-neutral-200 dark:border-slate-800 px-4 py-2.5 text-sm text-neutral-500 dark:text-slate-400 hover:bg-neutral-50 dark:hover:bg-slate-800/60"
            >
              Cancelar
            </button>
          </div>
