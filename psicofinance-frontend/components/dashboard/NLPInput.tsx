...
// Sugerencias de ejemplo para orientar al usuario
const SUGERENCIAS = [
...
export default function NLPInput({ onTurnoCreado }: Props) {
  ...
  return (
    <section className="overflow-hidden rounded-2xl bg-slate-50 shadow-sm ring-1 ring-slate-100">
      ...
      <div className="flex items-center justify-between px-5 pt-4 pb-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-sm font-semibold text-neutral-900">Copiloto</p>
            <p className="text-xs text-neutral-400">Registrá turnos o consultá tus finanzas</p>
          </div>
        </div>
        {cargando && (
      ...
      {/* ── Estado idle: chips de sugerencia ── */}
      {idle && (
        <div className="flex flex-wrap gap-2 px-5 pb-4">
          {SUGERENCIAS.map((s) => (
            <button
              ...
              className="rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs text-neutral-500 transition-colors hover:border-violet-300 hover:bg-violet-50 hover:text-violet-700"
            >
              {s}
            </button>
          ))}
      ...
      {respuesta && (
        <div className="px-4 pb-2">
          <div className="max-w-[92%] rounded-2xl rounded-tl-sm bg-neutral-100 px-4 py-3">
      ...
      {/* ── Error ── */}
      {error && (
        <div className="mx-4 mb-3 rounded-xl bg-red-50 px-3 py-2.5">
      ...
      {adjunto && (
        <div className="mx-4 mb-3">
          <div
            ...
            className="flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-dashed border-neutral-200 bg-neutral-50 py-4 transition-colors hover:bg-neutral-100"
          >
            {cargando
              ? <span className="h-4 w-4 animate-spin rounded-full border border-neutral-300 border-t-neutral-600" />
              : <>
                  <Paperclip className="h-4 w-4 text-neutral-400" />
                  <span className="text-sm text-neutral-500">Seleccioná el comprobante</span>
                  <span className="text-xs text-neutral-300">JPG · PNG · PDF</span>
                </>
            }
          </div>
      ...
      {/* ── Barra de input ── */}
      <div className="flex items-end gap-2 border-t border-slate-100 bg-white px-3 py-2.5">
        <button
          onClick={() => setAdjunto(v => !v)}
          className={`mb-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-xl transition-colors ${
            adjunto ? "bg-neutral-200 text-neutral-700" : "text-neutral-300 hover:text-neutral-500"
          }`}
        ...
        {!cargando && (
          <button
            onClick={enviar}
            disabled={!texto.trim()}
            className="mb-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-neutral-900 text-white transition-all hover:bg-neutral-700 disabled:cursor-not-allowed disabled:opacity-0"
          >
