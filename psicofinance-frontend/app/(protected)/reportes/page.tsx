  ...
  return (
    <main className="mx-auto max-w-screen-lg px-4 py-6 lg:py-8">
      {!cargando && errorCarga && (
        <div className="mb-4 flex items-center justify-between rounded-2xl bg-red-50 px-4 py-3 ring-1 ring-red-100 dark:bg-red-500/10 dark:ring-red-500/20">
          ...
          <button onClick={cargarTodo}
            className="rounded-xl border border-red-200 bg-white px-3 py-1.5 text-xs font-medium text-red-600 hover:bg-red-50 dark:border-red-500/20 dark:bg-slate-900 dark:text-red-400 dark:hover:bg-red-500/10">
            Reintentar
          </button>
        </div>
      )}
      ...
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h1 className="bg-gradient-to-r from-neutral-900 via-indigo-800 to-neutral-900 bg-clip-text text-xl font-extrabold tracking-tight text-transparent dark:from-slate-100 dark:via-indigo-300 dark:to-slate-100">Reportes</h1>
          <p className="text-xs text-neutral-500 dark:text-slate-400">Análisis financiero del consultorio · año {anioActual}</p>
        </div>
        <div className="flex flex-col gap-1.5 sm:items-end print:hidden">
          <div className="flex flex-wrap items-center gap-2">
            <button
              onClick={() => window.print()}
              className="flex shrink-0 items-center gap-1.5 rounded-xl border border-neutral-200 bg-white px-3 py-2 text-xs font-medium text-neutral-600 shadow-sm transition hover:bg-neutral-50 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-400 dark:hover:bg-slate-800/60"
          ...
          <div className="flex flex-wrap items-center gap-1.5 rounded-xl border border-dashed border-neutral-200 bg-neutral-50/60 px-2 py-1.5 dark:border-slate-800 dark:bg-slate-950/40">
            <span className="text-[11px] font-medium text-neutral-500 dark:text-slate-400">Exportar CSV:</span>
            <span className="text-[11px] text-neutral-400 dark:text-slate-500">desde</span>
            <input
              type="month"
              value={exportDesde}
              max={exportHasta}
              onChange={e => setExportDesde(e.target.value)}
              className="rounded-lg border border-neutral-200 bg-white px-2 py-1.5 text-xs text-neutral-700 focus:outline-none focus:ring-2 focus:ring-emerald-500 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300"
            />
            <span className="text-[11px] text-neutral-400 dark:text-slate-500">hasta</span>
            <input
              type="month"
              value={exportHasta}
              min={exportDesde}
      ...
      <div className="mb-5 grid grid-cols-2 gap-3 lg:grid-cols-4">
        {[
          ...
          <div key={label} className="relative overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-black/5 dark:bg-slate-900 dark:ring-white/10">
            ...
            <div className="p-4">
              ...
              {cargando
                ? <div className="mt-3 h-7 w-24 animate-pulse rounded bg-neutral-100 dark:bg-slate-800" />
      ...
      <div className="mb-4 rounded-2xl bg-white p-5 shadow-sm ring-1 ring-black/5 dark:bg-slate-900 dark:ring-white/10">
        <div className="mb-4 flex items-baseline justify-between">
          <div>
            <p className="text-[11px] font-medium uppercase tracking-wider text-neutral-400 dark:text-slate-500">Ingresos mensuales</p>
            <p className="mt-1 text-lg font-bold tabular-nums text-neutral-900 dark:text-slate-100">
              {metricas ? fmtPesos(metricas.ventas_mensuales.reduce((s, v) => s + v.cobrado, 0)) : "—"}
            </p>
          </div>
          <span className="text-xs text-neutral-400 dark:text-slate-500">Últimos 6 meses</span>
        </div>
        {cargando ? (
          <div className="h-48 animate-pulse rounded-xl bg-neutral-100 dark:bg-slate-800" />
        ) : (
      ...
      <div className="mb-4 rounded-2xl bg-white p-5 shadow-sm ring-1 ring-black/5 dark:bg-slate-900 dark:ring-white/10">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-neutral-600 dark:text-slate-400">¿Estás ganándole a la inflación?</p>
          </div>
        ...
        {cargando ? (
          ...
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={dataLineas} margin={{ top: 4, right: 12, left: 0, bottom: 0 }}>
              ...
              <Legend iconType="circle" iconSize={8} wrapperStyle={{ fontSize: 10 }} formatter={(value) => <span className="text-[10px] sm:text-xs text-neutral-600 dark:text-slate-400">{value}</span>} />
  ...
  return (
    <div className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-black/5 dark:bg-slate-900 dark:ring-white/10">
      <div className="mb-4 flex items-center justify-between">
        ...
        <span className="text-[10px] text-neutral-400 dark:text-slate-500">Criterio percibido</span>
      </div>
      {cargando ? (
        <div className="space-y-2">{[1,2,3,4].map(i => <div key={i} className="h-9 animate-pulse rounded-xl bg-neutral-100 dark:bg-slate-800" />)}</div>
      ) : (
        <div className="space-y-1">
          {lineas.map(({ label, valor, cls, signo }) => (
            <div key={label} className="flex items-center justify-between rounded-xl px-3 py-2 hover:bg-slate-50 dark:hover:bg-slate-800/60 transition-colors">
              <p className="text-sm text-neutral-600 dark:text-slate-400">{label}</p>
              <p className={`font-mono text-sm font-semibold tabular-nums ${cls}`}>
                {valor !== null ? `${signo}${fmtPesos(valor)}` : "—"}
              </p>
            </div>
          ))}
          <div className="mx-3 my-1 h-px bg-neutral-200 dark:bg-slate-800" />
          <div className="flex items-center justify-between rounded-xl bg-slate-50 dark:bg-slate-800/60 px-3 py-2.5">
            <p className="text-sm font-semibold text-neutral-800 dark:text-slate-100">Utilidad neta</p>
            <div className="text-right">
              ...
              {margen !== null && (
                <p className="text-[10px] tabular-nums text-neutral-400 dark:text-slate-500">margen {margen.toFixed(0)}%</p>
              )}
            </div>
          </div>
        </div>
...
function EgresosPorCategoria({ egresos }: { egresos: ResumenEgresos }) {
  ...
  return (
    <div className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-black/5 dark:bg-slate-900 dark:ring-white/10">
      <div className="mb-4 flex items-baseline justify-between">
        <p className="text-[11px] font-medium uppercase tracking-wider text-neutral-400 dark:text-slate-500">Egresos por categoría</p>
        <span className="text-xs text-neutral-400 dark:text-slate-500">Últimos 6 meses</span>
      </div>
      {!hayDatos ? (
        <p className="py-10 text-center text-sm text-neutral-400 dark:text-slate-500">Todavía no hay egresos registrados.</p>
      ) : (
        <ResponsiveContainer width="100%" height={220}>
...
function RankingPacientes({ top, total, cargando }: { top: PacienteConStats[]; total: number; cargando: boolean }) {
  return (
    <div className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-black/5 dark:bg-slate-900 dark:ring-white/10">
      <div className="mb-4 flex items-center justify-between">
        <p className="text-[11px] font-medium uppercase tracking-wider text-neutral-400 dark:text-slate-500">Top 5 pacientes</p>
        <Award className="h-4 w-4 text-amber-400" />
      </div>
      {cargando ? (
        <div className="space-y-2">{[1,2,3,4,5].map(i=><div key={i} className="h-12 animate-pulse rounded-xl bg-neutral-100 dark:bg-slate-800"/>)}</div>
      ) : top.length === 0 ? (
        <p className="py-6 text-center text-sm text-neutral-400 dark:text-slate-500">Sin datos suficientes.</p>
      ) : (
            ...
            return (
              <div key={p.id} className="flex items-center gap-3">
                <span className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-[11px] font-bold ring-1 ${
                  ...
                  : "bg-neutral-100 text-neutral-500 ring-neutral-200 dark:bg-slate-800 dark:text-slate-400 dark:ring-slate-700"
                }`}>{i+1}</span>
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-baseline justify-between gap-2">
                    <p className="truncate text-sm font-medium text-neutral-800 dark:text-slate-100">{p.nombre} {p.apellido}</p>
                  ...
                  <div className="mt-1 flex items-center gap-2">
                    ...
                    <span className="shrink-0 text-[10px] tabular-nums text-neutral-400 dark:text-slate-500">{pct.toFixed(0)}%</span>
                  </div>
                </div>
              </div>
            );
...
function DistribucionActividad({ d, cargando }: { d: { activos: number; moderados: number; inactivos: number; total: number }; cargando: boolean }) {
  ...
  return (
    <div className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-black/5 dark:bg-slate-900 dark:ring-white/10">
      <div className="mb-4 flex items-center justify-between">
        <p className="text-[11px] font-medium uppercase tracking-wider text-neutral-400 dark:text-slate-500">Distribución de pacientes</p>
        <Target className="h-4 w-4 text-indigo-400" />
      </div>
      {cargando ? (
        <div className="space-y-3">
          <div className="h-3 w-full animate-pulse rounded-full bg-neutral-100 dark:bg-slate-800" />
          <div className="space-y-2 mt-4">{[1,2,3].map(i=><div key={i} className="h-10 animate-pulse rounded-xl bg-neutral-100 dark:bg-slate-800"/>)}</div>
        </div>
      ) : d.total === 0 ? (
        <p className="py-6 text-center text-sm text-neutral-400 dark:text-slate-500">Sin pacientes registrados.</p>
      ) : (
        <>
          <div className="mb-4 flex h-3 overflow-hidden rounded-full bg-neutral-100 dark:bg-slate-800">
            {segs.map(s => (
          ...
          <div className="space-y-2.5">
            {segs.map(s => (
              <div key={s.label} className="flex items-center justify-between rounded-xl px-3 py-2 hover:bg-slate-50 dark:hover:bg-slate-800/60 transition-colors">
                <div className="flex items-center gap-2.5">
                  <span className={`h-2 w-2 shrink-0 rounded-full ${s.color}`} />
                  <div>
                    <p className="text-sm font-medium text-neutral-800 dark:text-slate-100">{s.label}</p>
                    <p className="text-[10px] text-neutral-400 dark:text-slate-500">{s.desc}</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className={`text-base font-bold tabular-nums ${s.text}`}>{s.n}</p>
                  <p className="text-[10px] tabular-nums text-neutral-400 dark:text-slate-500">{((s.n/total)*100).toFixed(0)}%</p>
                </div>
              </div>
            ))}
          </div>
  ...
  return (
    <div className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-black/5 dark:bg-slate-900 dark:ring-white/10">
      <div className="mb-3 flex items-center justify-between gap-3 flex-wrap">
        <p className="text-[11px] font-medium uppercase tracking-wider text-neutral-400 dark:text-slate-500">Estado fiscal — Monotributo</p>
        {/* Selector de categoría */}
        {semaforo && (
          <div className="flex items-center gap-2">
            <span className="text-[11px] text-neutral-400 dark:text-slate-500">Categoría:</span>
                ...
                <button
                  ...
                  className={`h-8 w-8 sm:h-6 sm:w-6 rounded-md text-[11px] font-bold transition-all disabled:opacity-50 ${
                    cat === semaforo.categoria_actual
                      ...
                      : "bg-neutral-100 text-neutral-500 hover:bg-indigo-50 hover:text-indigo-600 dark:bg-slate-800 dark:text-slate-400 dark:hover:bg-indigo-500/10 dark:hover:text-indigo-400"
                  }`}
                >
                  {cat}
                </button>
      ...
      {cargando ? (
        <div className="h-20 animate-pulse rounded-xl bg-neutral-100 dark:bg-slate-800" />
      ) : semaforo ? (
        <div className="flex flex-wrap items-center gap-6">
          <div>
            <span className="text-3xl font-bold text-neutral-900 dark:text-slate-100">Cat. {semaforo.categoria_actual}</span>
          ...
          <div className="flex-1 min-w-48">
            ...
            <div className="mt-2 flex justify-between text-xs text-neutral-500 dark:text-slate-400">
              <span>Facturado 12m: <strong className="text-neutral-800 dark:text-slate-100">{fmtPesos(semaforo.facturado_12m)}</strong></span>
              <span>Tope: <strong className="text-neutral-800 dark:text-slate-100">{fmtPesos(semaforo.tope_anual)}</strong></span>
            </div>
          </div>
